...
Val_humanObserved_Target_Concat = GenerateValTargetVector(humanObserved_Target_Concat, ValidationPercent, (len(Training_humanObserved_Target_Concat)))
Val_humanObserved_Features_Concat = GenerateValData(humanObserved_Features_Concat, ValidationPercent, (len(Training_humanObserved_Target_Concat)))
...
Test_humanObserved_Target_Concat = GenerateValTargetVector(humanObserved_Target_Concat, TestPercent, (len(Training_humanObserved_Target_Concat) + len(Val_humanObserved_Target_Concat)))
Test_humanObserved_Features_Concat = GenerateValData(humanObserved_Features_Concat, TestPercent, (len(Training_humanObserved_Target_Concat) + len(Val_humanObserved_Target_Concat)))
...
Val_humanObserved_Target_Sub = GenerateValTargetVector(humanObserved_Target_Sub, ValidationPercent, (len(Training_humanObserved_Target_Sub)))
Val_humanObserved_Features_Sub = GenerateValData(humanObserved_Features_Sub, ValidationPercent, (len(Training_humanObserved_Target_Sub)))
...
Test_humanObserved_Target_Sub = GenerateValTargetVector(humanObserved_Target_Sub, TestPercent, (len(Training_humanObserved_Target_Sub) + len(Val_humanObserved_Target_Sub)))
Test_humanObserved_Features_Sub = GenerateValData(humanObserved_Features_Sub, TestPercent, (len(Training_humanObserved_Target_Sub) + len(Val_humanObserved_Target_Sub)))
...
GSCObserved_Features_Concat, GSCObserved_Features_Concat_length = generate_raw_data_with_relevant_features(GSCObserved_Features_Concat,\
                                                                                                           is_synthetic=False)
GSCObserved_Features_Sub, GSCObserved_Features_Sub_length = generate_raw_data_with_relevant_features(GSCObserved_Features_Sub,\
                                                                                                           is_synthetic=False)
...
Val_GSCObserved_Target_Concat = GenerateValTargetVector(GSCObserved_Target_Concat, ValidationPercent, (len(Training_GSCObserved_Target_Concat)))
Val_GSCObserved_Features_Concat = GenerateValData(GSCObserved_Features_Concat, ValidationPercent, (len(Training_GSCObserved_Target_Concat)))
...
Test_GSCObserved_Target_Concat = GenerateValTargetVector(GSCObserved_Target_Concat, TestPercent, (len(Training_GSCObserved_Target_Concat) + len(Val_GSCObserved_Target_Concat)))
Test_GSCObserved_Features_Concat = GenerateValData(GSCObserved_Features_Concat, TestPercent, (len(Training_GSCObserved_Target_Concat) + len(Val_GSCObserved_Target_Concat)))
...
Val_GSCObserved_Target_Sub = GenerateValTargetVector(GSCObserved_Target_Sub, ValidationPercent, (len(Training_GSCObserved_Target_Sub)))
Val_GSCObserved_Features_Sub = GenerateValData(GSCObserved_Features_Sub, ValidationPercent, (len(Training_GSCObserved_Target_Sub)))
...
Test_GSCObserved_Target_Sub = GenerateValTargetVector(GSCObserved_Target_Sub, TestPercent, (len(Training_GSCObserved_Target_Sub) + len(Val_GSCObserved_Target_Sub)))
Test_GSCObserved_Features_Sub = GenerateValData(GSCObserved_Features_Sub, TestPercent, (len(Training_GSCObserved_Target_Sub) + len(Val_GSCObserved_Target_Sub)))
...
def linear_regression_solution(training_phi_matrix,training_target,M,LR,lambda_value):
    ...
    for i in range(NUMBER_OF_ITERATIONS):
        ...
        calculated_gradient = (1/len(training_phi_matrix)) * np.dot(np.transpose(calculated_error),training_phi_matrix)
...
def linear_regression_human_concat(M,LR,lambda_value):
    concat_big_sigma = generate_big_sigma_matrix(humanObserved_Features_Concat, number_of_features=human_concat, is_synthetic=False)
    concat_big_sigma_inverse = np.linalg.inv(concat_big_sigma)
    Training_Human_Concat_Mean = KMeans(n_clusters=M, random_state=0).fit(humanObserved_Features_Concat)
    ...
    concat_human_weight_matrix = linear_regression_solution(training_human_phi_matrix,Training_humanObserved_Target_Concat,M,LR,lambda_value)
...
def linear_regression_human_sub(M,LR,lambda_value):
    ...
    concat_human_weight_matrix = linear_regression_solution(training_human_phi_matrix,Training_humanObserved_Target_Sub,M,LR,lambda_value)
...
def linear_regression_gsc_concat(M,LR,lambda_value):
    concat_big_sigma = generate_big_sigma_matrix(GSCObserved_Features_Concat, number_of_features=GSCObserved_Features_Concat_length-1,
                                                 is_synthetic=False)
    concat_big_sigma_inverse = np.linalg.inv(concat_big_sigma)
    ...
    concat_human_weight_matrix = linear_regression_solution(training_GSC_phi_matrix,Training_GSCObserved_Target_Concat,M,LR,lambda_value)
...
def linear_regression_gsc_sub(M,LR,lambda_value):
    sub_big_sigma = generate_big_sigma_matrix(GSCObserved_Features_Sub, number_of_features=GSCObserved_Features_Sub_length-1,
                                                 is_synthetic=False)
    ...
    concat_human_weight_matrix = linear_regression_solution(training_gsc_phi_matrix,Training_GSCObserved_Target_Sub,M,LR,lambda_value)
...
def cal_erms_acc_human_concat_m_values():
    ...
    for i in range(len(M)):
        training_output, validation_output, testing_output = linear_regression_human_concat(M[i],LR=0.01,lambda_value=2)
        training_erms, training_accuracy = compute_rms_error_and_accuracy(training_output, Training_humanObserved_Target_Concat)
        validation_erms, validation_accuracy = compute_rms_error_and_accuracy(validation_output, Val_humanObserved_Target_Concat)
        testing_erms, testing_accuracy = compute_rms_error_and_accuracy(testing_output, Test_humanObserved_Target_Concat)
...
def cal_erms_acc_human_concat_LR_values():
    ...
    for i in range(len(learningRate)):
        training_output, validation_output, testing_output = linear_regression_human_concat(10,learningRate[i],lambda_value=2)
        training_erms, training_accuracy = compute_rms_error_and_accuracy(training_output, Training_humanObserved_Target_Concat)
        validation_erms, validation_accuracy = compute_rms_error_and_accuracy(validation_output,Val_humanObserved_Target_Concat)
        testing_erms, testing_accuracy = compute_rms_error_and_accuracy(testing_output, Test_humanObserved_Target_Concat)
        Erms_TR.append(training_erms)
...
def cal_erms_acc_human_sub_LR_values():
    clear_list()
    for i in range(len(learningRate)):
        training_output, validation_output, testing_output = linear_regression_human_sub(10,learningRate[i],lambda_value=2)
        training_erms, training_accuracy = compute_rms_error_and_accuracy(training_output,
                                                                          Training_humanObserved_Target_Sub)
        validation_erms, validation_accuracy = compute_rms_error_and_accuracy(validation_output,
                                                                              Val_humanObserved_Target_Sub)
...
def cal_erms_acc_gsc_concat_LR_values():
    clear_list()
    for i in range(len(learningRate)):
        training_output, validation_output, testing_output = linear_regression_gsc_concat(10,learningRate[i],lambda_value=2)
        training_erms, training_accuracy = compute_rms_error_and_accuracy(training_output,
                                                                          Training_GSCObserved_Target_Concat)
        validation_erms, validation_accuracy = compute_rms_error_and_accuracy(validation_output,
                                                                              Val_GSCObserved_Target_Concat)
...
def cal_erms_acc_gsc_sub_LR_values():
    clear_list()
    for i in range(len(learningRate)):
        training_output, validation_output, testing_output = linear_regression_gsc_sub(10,learningRate[i],lambda_value=2)
        training_erms, training_accuracy = compute_rms_error_and_accuracy(training_output,
                                                                          Training_GSCObserved_Target_Sub)
        validation_erms, validation_accuracy = compute_rms_error_and_accuracy(validation_output,
                                                                              Val_GSCObserved_Target_Sub)
